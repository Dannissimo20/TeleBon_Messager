...
// eslint-disable-next-line import/order
import { ModalContainer } from './form-start/FirstForm.styled';
...
const FormElement: FC<IProps> = observer((props) => {
  ...
  const handleBlockTime = async () => {
    ...
    const allPaymentsComplete = (formik.values.clientId || []).every((item: { paymentComplete: string }) => item.paymentComplete === 'yes');
    ...
    return (
      <FlexWithAlign $justify='between'>
        ...
        <FlexWithAlign>
          ...
          <>
            {!recordingStore?.isId && recordingStore?.chat === 2 && (
              <CommonButton
                typeBtn='primary'
